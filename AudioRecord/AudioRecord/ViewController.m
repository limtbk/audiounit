...
@property (nonatomic, assign) UInt32 maxFramesPerSlice;
...
{
    TRYR(AudioUnitRender(sharedViewController.audioUnit, ioActionFlags, inTimeStamp, 1, inNumberFrames, ioData));
    ...
    for (UInt32 i=0; i<ioData->mNumberBuffers; ++i) {
        ...
        printf("l: %d t: %f MIN: %f  MAX: %f  AVG: %f\n", inNumberFrames, inTimeStamp->mSampleTime/sharedViewController.sampleRate, min, max, avg);
//        memset(ioData->mBuffers[i].mData, 0, ioData->mBuffers[i].mDataByteSize);
        for (NSUInteger frame = 0; frame < inNumberFrames; frame++) {
            double t = (inTimeStamp->mSampleTime + frame)/sharedViewController.sampleRate;
            if (t - floor(t)<0.01) {
...
-(OSStatus) setupAudioSession {
    ...
    self.bufferDuration = .005; //5 ms, 220.5 samples in 44100
    self.sampleRate = 44100;

    TRYE([sessionInstance setCategory:AVAudioSessionCategoryPlayAndRecord error:&error]);
    TRYE([sessionInstance setPreferredIOBufferDuration:self.bufferDuration error:&error]);
    TRYE([sessionInstance setPreferredSampleRate:self.sampleRate error:&error]);
...
- (OSStatus)setupIOUnit
{
    ...
    UInt32 one = 1;
    TRYR(AudioUnitSetProperty(self.audioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1, &one, sizeof(one)));
    TRYR(AudioUnitSetProperty(self.audioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &one, sizeof(one)));
    
    AudioStreamBasicDescription ioFormat = [self audioStreamBasicDescription];
    
    TRYR(AudioUnitSetProperty(self.audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &ioFormat, sizeof(ioFormat)));
    TRYR(AudioUnitSetProperty(self.audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &ioFormat, sizeof(ioFormat)));
    
    UInt32 maxFramesPerSlice = 4096;
    UInt32 maxFramesPerSliceSize = sizeof(maxFramesPerSlice);
    TRYR(AudioUnitSetProperty(self.audioUnit, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, 0, &maxFramesPerSlice, maxFramesPerSliceSize));
    TRYR(AudioUnitGetProperty(self.audioUnit, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, 0, &maxFramesPerSlice, &maxFramesPerSliceSize));
    ...
    TRYR(AudioUnitSetProperty(self.audioUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, 0, &renderCallback, sizeof(renderCallback)));
    TRYR(AudioUnitInitialize(self.audioUnit));
    return noErr;
}
...
{
    TRYR(AudioOutputUnitStart(self.audioUnit));
    return noErr;
}

- (OSStatus)stopIOUnit
{
    TRYR(AudioOutputUnitStop(self.audioUnit));
    return noErr;
}
